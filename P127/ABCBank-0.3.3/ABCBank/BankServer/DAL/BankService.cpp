...
int BankService::QueryDayBill(list<TransDetail>& result, int page, const string& date)
{
	...
	明细查询最关键的sql语句
	查询语句
	date_format(trans_date, '%Y-%m-%d')='2013-08-02')格式化时间，满足2013-08-02的就可以了；
	abstract摘要表；
	还需要与摘要表进行连接，获取摘要名称
	...
	返回给客户端，以便客户端能够直接的显示出来，不需要根据摘要id去转换，因为他根本就不知道，只有服务器端知道这些信息
	*/
	...
	try
	{
		...
		for (unsigned int i=0; i<rs.GetRows(); ++i)
		{
			...
			//把查询出来的明细添加到result，result是一个列表list
			result.push_back(td);
		}
	}
	catch (Exception& e)
