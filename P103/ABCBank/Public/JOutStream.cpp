...
{
	int16 be16 = Endian::HostToNetwork16(x);//用于网络，先转换成网络字节序
	Append(&be16, sizeof be16);
	return *this;
}
...
JOutStream& JOutStream::operator<<(int32 x)
{
	...
	Append(&be32, sizeof be32);//调用void JOutStream::Append(const void*  data, size_t len)
	return *this;
}

JOutStream& JOutStream::operator<<(const std::string& str)
{
	uint16 len = static_cast<uint16>(str.length());//str.length()是字符串的长度
	*this<<len;//先打包2个字节数
	Append(str.c_str(), len);//再打包字符串，调用void JOutStream::Append(const char* data, size_t len)
	return *this;
}
...
void JOutStream::Append(const char* data, size_t len)
{
	...
	//将data数据拷贝到buffer_
	std::copy(data, data+len, buffer_.begin()+currIndex_);//真正的打包
	currIndex_ += len;
}
