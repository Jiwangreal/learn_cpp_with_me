...
{
	if (tcp)
		socket_ = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);//流失套接字
	else
		socket_ = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);//数据包套接字
	assert(INVALID_SOCKET != socket_);

	int optval = 1;
	setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR,
		(const char*)&optval, sizeof optval);//设置地址重复利用
}
...
int Socket::SendN(const char* buf, size_t len)
{
	...
	return len - nLeft;//没法送完，返回值一定<len
}
