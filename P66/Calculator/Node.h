...
//构造函数初始化，要调用基类部分的构造函数
    AssignNode(std::auto_ptr<Node>& left, std::auto_ptr<Node>& right)
        ...
        : left_(left), right_(right) {}
            ...
            //bug原因：将left构造BinaryNode基类，会调用基类的构造函数。用left初始化left_。left的所有权转移到了left_，此时left内部所
            //持有的指针是空指针，所以调用assert(left->IsLvalue())会出错
            以前没有用智能指针，所以没出问题，现在用到了智能指针，会涉及到控制权转移问题，所以出了问题
            //改为：用其成员断言
           ssert(left_->IsLvalue());//断言是一个左值节点，能够被赋值的 
            */
