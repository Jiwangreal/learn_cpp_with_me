...
MysqlRecordset MysqlDB::QuerySQL(const char* sql)
{
	...
	MYSQL_RES*  mysql_res;
	mysql_res = mysql_store_result(mysql_);//返回结果集
	...
	MysqlRecordset rs;
	unsigned int i = 0;
	unsigned int nCols = mysql_num_fields(mysql_res);//字段的个数
	...
	MYSQL_ROW mysql_row;
	while ((mysql_row = mysql_fetch_row(mysql_res)))//取出行
	{
...
unsigned long long MysqlDB::ExecSQL(const char* sql)
{
	...
	return mysql_affected_rows(mysql_);//返回影响的行数
