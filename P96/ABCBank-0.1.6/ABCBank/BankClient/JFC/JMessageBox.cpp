...
{
	//第一个按钮到最后一个按钮的距离
	short btn_distance = static_cast<short>(buttons.size()) - 1;//按钮和按钮之间空格的距离

	for (int i=0; i<static_cast<int>(buttons.size()); ++i)
		btn_distance += buttons[i].length() + 4;		// 每个按钮宽度=文本长度+4，4指的是前面的2个空格
														//和后面的2个空格
	...
	JWindow* w = jApp->GetCurrent();//先保存当前的按钮

	//当消息框退出时，再将其设置为当前按钮
	JRECT rect = {x, y, x+width-1, y+height-1};
	ScreenSaver sc(rect);
	sc.Dump();		// 保存被消息框遮盖部分的屏幕数据
	jmbl.Show();//消息框显示，调用JMessageBoxImpl::Draw()函数来绘制
	int ret = jmbl.Exec();//进入消息框循环
