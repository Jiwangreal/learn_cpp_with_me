        ...
        if (scanner_.Token() == TOKEN_LPARENTHESIS)
        {
            ...
            if (scanner_.Token() == TOKEN_RPARENTHESIS)//accept ')'
            {
                ...
                else
                {
                    ...
                    char buf[256] = {0};
                    // sprintf(buf, "Unknow function \" %s \".", symbol.c_str());//symbol是string类型，symbol.c_str()是char*类型
                    // throw SyntaxError(buf);
                   
                    ostringstream oss;
                    std::oss<<"Unknow function \"<<symbol<<"\"."; //已经格式化到字符串输出流的缓冲区中了
                    throw SyntaxError(oss.str());//oss.str()返回的类型是string类型
                }
