...
void OpenAccountForm::Submit()
{
	...
	{
		//搜集数据到BankSession
		//BankSession是与服务器端通信的会话
		//这里不进行打包，打包交给了CMD层来完成，不同的命令有不同的打包方式，eg：开户的请求包体只有4项
		BankSession bs;
		//命令是开户操作
		...
		bs.SetAttribute("id", editId_->GetText());//身份证号码
		bs.SetAttribute("money", editMoney_->GetText());//开户金额

		//调用交易管理器类TransactionManager找到CMD层的对应的交易，并调用其Execute函数
		...
		if (bs.GetErrorCode() == 0)
		{
			...
			//将这些信息放在回执单中
			//设置成功，则弹出回执单，FormManager会获取开户的回执单对象
			//所以FormManager会管理一些界面，所以需要将回执单界面放到FormManager中去管理
			ReceiptForm* form;
			form = dynamic_cast<ReceiptForm*>(Singleton<FormManager>::Instance().Get("OpenAccountReceiptForm"));
			form->SetReceiptFormType(ReceiptForm::RFT_OPEN_ACCOUNT);//设置回执单的类型
			//设置回执单的项并显示出来
